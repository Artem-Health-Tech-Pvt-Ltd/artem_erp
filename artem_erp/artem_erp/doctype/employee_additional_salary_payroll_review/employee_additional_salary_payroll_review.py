...
class EmployeeAdditionalSalaryPayrollReview(Document):
	...
	def validate_is_reviewed_completion(self):
		...
		if wants_completed:
			unreviewed_current = [
				row for row in (self.additional_salary_payment_details or []) if not cint(row.is_reviewed)
			]
			unreviewed_pending = [
				row for row in (self.previous_pending_additional_salary or []) if not cint(row.is_reviewed)
			]
